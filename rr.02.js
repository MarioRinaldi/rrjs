...
var rr = {
    ...
    popupFrame_hide: function() {
        document.body.removeChild(rr.getElement('rrPopupFrame')); 
		return true;
    },
    
    createOverlay: function (elem, v) {
        var vars = rr.getVars(v, {className: 'rrOverlay'} );

        var hideOverlay = function () { rr.removeElement(overlay); return true; };
        ...
        rr.addEvent(elem, "click", hideOverlay);
    ...
    ajax: {
        ...
        restoreFromUrl: function (url, v) {
            ...
            rr.ajax.restoreContent(args[elems],args[urls])
        },
        
        restoreContent: function (elems, urls, v) {
            var arrElems = join(',',elems);
            var arrElems = join(',',elems);
        }
    },
